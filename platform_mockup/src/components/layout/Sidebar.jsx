...
const navItems = [
  ...
  { path: '/fans', icon: Users, label: 'Fan CRM' },
  { path: '/revenue', icon: TrendingUp, label: 'Revenue' },
  { path: '/gamification', icon: Gamepad2, label: 'Gamification' },
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 bg-altyr-bg-dark border-r border-white/[0.08] z-50 flex flex-col">
      {/* Logo */}
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path || 
            (item.path === '/fans' && location.pathname.startsWith('/fans/'))
          ...
          return (
            ...
            >
              ...
              >
                <item.icon 
                  className={`w-5 h-5 ${isActive ? 'text-altyr-magenta' : 'text-white/50'}`} 
                />
                <span 
                  className={`text-sm font-light ${isActive ? 'text-white' : 'text-white/60'}`}
                >
                  {item.label}
                </span>
