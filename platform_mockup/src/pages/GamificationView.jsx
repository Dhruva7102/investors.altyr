...
import { Zap, Star, Award, Calendar, Trophy, Eye, MessageSquare, CreditCard, Heart, Users } from 'lucide-react'
...
const iconMap = {
  Zap, Eye, MessageSquare, Heart, Star, CreditCard, Users, Award
}
      ...
      <GlassCard className="bg-gradient-to-r from-altyr-magenta/10 to-altyr-purple/5">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-altyr-bg border border-altyr-magenta/50">
              <span className="text-xs text-altyr-magenta font-medium">Level {demoFanProfile.level}</span>
            </div>
          </div>
          ...
          <div className="flex-1 w-full min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-3">
              <span className="text-sm text-white/60">
                Progress to Level {demoFanProfile.level + 1}
              </span>
            ...
            {dailyLoginRewards.map((day, index) => (
              <motion.div
                ...
                className={`
                  ...
                  ${day.claimed 
                    ? 'bg-gradient-to-br from-altyr-magenta/20 to-altyr-purple/10 border border-altyr-magenta/30' 
                    : 'bg-white/[0.02] border border-white/[0.08]'
                  }
                `}
              >
                <p className="text-xs text-white/40 mb-1">Day {day.day}</p>
                <p className={`text-sm font-medium ${day.claimed ? 'text-gradient' : 'text-white/60'}`}>
                  {day.reward}
                </p>
                {day.claimed && (
                  <div className="mt-2">
                      ...
                      <motion.div
                        ...
                        className={`
                          ...
                          ${badge.unlocked 
                            ...
                            : 'bg-white/[0.02] border border-white/[0.06] opacity-50'
                          }
                        `}
                      >
                        <div className={`text-2xl mb-2 ${badge.unlocked ? colors.text : 'text-white/20'}`}>
                          🏆
                        </div>
                        <p className={`text-xs font-medium mb-1 ${badge.unlocked ? 'text-white/90' : 'text-white/40'}`}>
                          {badge.name}
                        </p>
            ...
            {levelRewards.map((reward, index) => (
              <motion.div
                ...
                className={`
                  ...
                  ${reward.unlocked 
                    ? 'bg-gradient-to-r from-altyr-magenta/10 to-transparent border border-altyr-magenta/20' 
              ...
              >
                <div className={`
                  ...
                  ${reward.unlocked 
                    ? 'bg-gradient-to-br from-altyr-magenta to-altyr-purple text-white' 
                ...
                <div>
                  <p className={`text-sm ${reward.unlocked ? 'text-white/90' : 'text-white/50'}`}>
                    {reward.reward}
                  </p>
                  {reward.unlocked && (
