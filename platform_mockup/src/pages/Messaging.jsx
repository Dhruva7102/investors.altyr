...
import { GlassCard, StatusBadge, ConnectionScoreBar, IconContainer } from '@/components/shared'
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${isActive 
          ? 'bg-gradient-to-r from-altyr-magenta/20 to-altyr-purple/10 border border-altyr-magenta/30' 
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="relative flex-shrink-0">
          <div className={`
            ...
            bg-gradient-to-br ${statusColors[conversation.fanStatus]}
          `}>
            {conversation.fanInitials}
          </div>
          {/* Online indicator */}
          {conversation.isOnline && (
            <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-altyr-bg-dark" />
          )}
          {/* Level badge */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              ...
              {conversation.fanStatus === 'Superfan' && (
                <Crown className="w-3.5 h-3.5 text-amber-400" />
              )}
              {conversation.fanStatus === 'VIP' && (
                <Star className="w-3.5 h-3.5 text-altyr-magenta" />
              )}
            </div>
...
function MessageBubble({ message, isCreator }) {
  ...
  return (
    <motion.div
      ...
      className={`flex ${isCreator ? 'justify-end' : 'justify-start'} mb-3`}
    >
      <div className={`max-w-[70%] ${isCreator ? 'order-2' : ''}`}>
        ...
        <div className={`
          ...
          ${isCreator 
            ? 'bg-gradient-to-r from-altyr-magenta to-altyr-purple text-white rounded-br-sm' 
        ...
        <div className={`flex items-center gap-1 mt-1 ${isCreator ? 'justify-end' : ''}`}>
          ...
          {isCreator && message.read && (
            <span className="text-[10px] text-altyr-purple-light">✓✓</span>
          )}
        </div>
      </div>
    </motion.div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="text-center pb-4 border-b border-white/[0.08]">
          <div className="relative inline-block mb-3">
            ...
            {conversation.isOnline && (
              <div className="absolute bottom-1 right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-altyr-bg-dark" />
            )}
          </div>
        ...
        <div className="p-4 rounded-xl bg-gradient-to-br from-altyr-purple/10 to-altyr-magenta/5 border border-altyr-purple/20">
          ...
          <div className="space-y-2 text-xs text-white/60">
            <div className="flex items-start gap-2">
              ...
              <span>Best response time: <span className="text-white/80">7-9 PM</span></span>
            </div>
            <div className="flex items-start gap-2">
              <div className="w-1 h-1 rounded-full bg-altyr-orange mt-1.5 flex-shrink-0" />
              <span>Prefers: <span className="text-white/80">Personal touches, exclusivity</span></span>
            </div>
            <div className="flex items-start gap-2">
              <div className="w-1 h-1 rounded-full bg-green-400 mt-1.5 flex-shrink-0" />
              <span>Engagement trend: <span className="text-green-400">↑ 12% this month</span></span>
            </div>
          </div>
        </div>
          ...
          <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
            ...
            <div className="flex flex-wrap gap-1">
              {fan.badges.slice(0, 4).map((badge, index) => (
                <span 
                  key={index}
                  className="px-2 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[10px]"
                >
                  {badge}
                </span>
              ))}
        ...
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="flex items-center gap-2">
            <AlertTriangle className={`w-4 h-4 ${
              fan.churnRisk === 'High' ? 'text-red-400' : 
              fan.churnRisk === 'Medium' ? 'text-amber-400' : 'text-green-400'
            }`} />
        ...
        {fan.preferences && (
          <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
            ...
            {fan.preferences.likes && fan.preferences.likes.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {fan.preferences.likes.map((like, index) => (
                  <span 
                    key={index}
                    className="px-2 py-1 rounded-full bg-altyr-magenta/10 border border-altyr-magenta/30 text-altyr-magenta text-[10px]"
                  >
                    {like}
                  </span>
                ))}
            ...
            {fan.preferences.notes && (
              <p className="text-xs text-white/50 italic">"{fan.preferences.notes}"</p>
            )}
          </div>
        )}
        ...
        <div className="p-4 border-b border-white/[0.08]">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              {totalUnread > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-altyr-magenta text-[10px] font-medium">
                  {totalUnread}
                </span>
              )}
            </div>
            ...
            {filters.map((f) => (
              <button
                ...
                className={`
                  ...
                  ${filter === f
                    ? 'bg-altyr-magenta/20 text-altyr-magenta border border-altyr-magenta/30'
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              {activeConversation?.isOnline && (
                <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-altyr-bg" />
              )}
            </div>
            <div>
              <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-white/40">
                {activeConversation?.isOnline ? 'Online' : 'Last seen ' + activeConversation?.lastMessageTime}
              </p>
            </div>
          </div>
          ...
          <AnimatePresence>
            {messages.map((message) => (
              <MessageBubble
                key={message.id}
                message={message}
                isCreator={message.sender === 'creator'}
              />
            ))}
          </AnimatePresence>
        </div>
