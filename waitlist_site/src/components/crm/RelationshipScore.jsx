  ...
  const calculateFactorScore = (factor) => {
    switch (factor.name) {
      case 'Engagement Frequency':
        return Math.min(100, Math.round(((fan.totalMessages || 0) / 300) * 100))
      case 'Interaction Quality':
        return fan.sentiment === 'positive' ? 85 : fan.sentiment === 'neutral' ? 60 : 40
      case 'Financial Engagement':
        return Math.min(100, Math.round(((fan.ltv || 0) / 5000) * 100))
      case 'Content Consumption':
        return Math.min(100, Math.round(((fan.totalPurchases || 0) / 50) * 100))
      case 'Platform Tenure':
        return Math.min(100, (fan.subscriptionMonths || 0) * 8)
      default:
        return 50
    }
  }
