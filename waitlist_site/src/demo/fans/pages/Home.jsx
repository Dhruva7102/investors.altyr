    ...
    <div className="space-y-4 sm:space-y-6">
      <GlassCard className="bg-gradient-to-r from-altyr-magenta/10 to-altyr-purple/5">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4 sm:gap-6">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-altyr-magenta to-altyr-purple flex items-center justify-center text-white text-lg sm:text-xl font-light flex-shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-white/40 truncate">
                ...
                <span className="hidden sm:inline"> • {demoCreator.tagline}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 w-full">
            ...
            <div className="flex items-center justify-between mt-2 text-[10px] sm:text-xs text-white/40">
              <span>Streak: {currentStreak} days</span>
              <span>Badges: {demoFanProfile.unlockedBadges}</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full lg:w-[320px]">
            <div className="p-3 sm:p-4 rounded-xl bg-white/[0.03] border border-white/[0.08]">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] sm:text-xs text-white/60">Points</span>
              </div>
              <div className="text-xl sm:text-2xl font-light text-gradient">{pointsBalance}</div>
              <div className="text-[10px] sm:text-xs text-white/40">Spend on perks</div>
            </div>
            <div className="p-3 sm:p-4 rounded-xl bg-white/[0.03] border border-white/[0.08]">
              <div className="flex items-center gap-2 mb-2">
                <IconContainer icon={Trophy} size="sm" />
                <span className="text-[10px] sm:text-xs text-white/60">Quest</span>
              </div>
              <div className="text-xl sm:text-2xl font-light text-white/90">{completedCount}/{totalCount}</div>
              <div className="text-[10px] sm:text-xs text-white/40">Today</div>
            </div>
          </div>
        </div>
      </GlassCard>
      ...
      <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
        <GlassCard className="lg:col-span-2" padding="p-0">
          <div className="p-4 sm:p-6 border-b border-white/[0.08] flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <IconContainer icon={Sparkles} size="sm" />
              <div className="min-w-0">
                <h3 className="text-base sm:text-lg font-light text-white/90">Daily Quests</h3>
                <p className="text-[10px] sm:text-xs text-white/50 truncate">Build habit and deepen connection</p>
              </div>
            </div>
            <span className="text-[10px] sm:text-xs text-white/50 flex-shrink-0">{completedCount}/{totalCount}</span>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-3 sm:gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        {isDone ? <CheckCircle2 className="w-4 h-4 text-green-400 flex-shrink-0" /> : <div className="w-4 h-4 rounded-full border border-white/20 flex-shrink-0" />}
                        <p className="text-sm font-medium text-white/90">{q.title}</p>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.06] text-white/50 border border-white/[0.08]">
                          {q.category}
                        </span>
                      </div>
                      <p className="text-[10px] sm:text-xs text-white/50 line-clamp-2">{q.description}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-sm text-gradient font-medium">+{q.xp} XP</div>
                      <div className="text-[10px] text-white/40 hidden sm:block">{q.reward}</div>
                    </div>
                  </div>
                </div>
              )
        ...
        <GlassCard padding="p-0">
          <div className="p-4 sm:p-6 border-b border-white/[0.08] flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-light text-white/90">Suggested Perk</h3>
                <p className="text-[10px] sm:text-xs text-white/50">Spend points for momentum</p>
              </div>
            </div>
          </div>
          <div className="p-4 sm:p-6 space-y-3">
            <div className="p-3 sm:p-4 rounded-xl bg-white/[0.03] border border-white/[0.08]">
              ...
              <p className="text-[10px] sm:text-xs text-white/50 mt-1">{nextSuggested.description}</p>
              <div className="mt-3 flex items-center justify-between">
                <span className="text-[10px] sm:text-xs text-white/50">Cost</span>
